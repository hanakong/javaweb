  ...
  <form name="myform" method="post" action="${ctp}/MemberUpdateOk.mem" class="was-validated">
    ...
    <div class="form-group">
     ...
     <c:forEach var="tempHobby" items="${varHobbys}" varStatus="st">
     		<input type="checkbox" class="form-check-input" name="hobby" value="${tempHobby}" <c:if test="${fn:contains(hobby,varHobbys[st.index])}">checked</c:if> />${tempHobby} &nbsp;&nbsp; 
     </c:forEach> <!-- index는 0,1,2,3,4~ count는 1,2,3,4,5  -->
     </div>
    </div>
    <div class="form-group">
