...
<html>
	<head>
	  ...
	  <script>
			...
			// 박스 추가버튼을 클릭할때마다 입력폼 동적 추가 : 각 TextField마다 고유한 id를 지정해준다.
			function inputBox() {
				idx++;
			...
			// 상품 등록 전에 기본적인 사항을 체크한 후 정상적인 값을 서버로 전송한다.
			function fCheck() {
		...
		<div class="container">
			...
			<form name="myform" method="post" action="${pageContext.request.contextPath}/t0420/Test7Ok">
				...
				<p>
					...
					<input type="button" value="입력창 모두 제거" onclick="reloadCheck()" class="btn btn-warning"/>
				</p>
				...
				<p><input type="button" value="상품가격계산" onclick="fCheck()" class="btn btn-danger form-control" /></p>
			</form>
		</div>
		<p><br/><p>
	</body>
